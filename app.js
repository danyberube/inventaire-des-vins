...
function initSAQStoreAutocomplete() {
  const input = document.getElementById('saqStoreInput');
  const dropdown = document.getElementById('saqStoreDropdown');
  if (!input || !dropdown) return;

  function renderOptions(filter) {
    dropdown.innerHTML = '';
    const q = (filter || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const groups = { '23': { label: 'Montreal et Sud du Quebec', items: [] }, '33': { label: 'Quebec et Est du Quebec', items: [] } };
    SAQ_STORES.forEach(([id, name, banner]) => {
      const text = `${name} ${banner} ${id}`.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      if (q && !text.includes(q)) return;
      const prefix = id.startsWith('23') ? '23' : '33';
      groups[prefix].items.push([id, name, banner]);
    });
    let hasResults = false;
    ['23', '33'].forEach(prefix => {
      const g = groups[prefix];
      if (g.items.length === 0) return;
      hasResults = true;
      const lbl = document.createElement('div');
      lbl.className = 'saq-store-group-label';
      lbl.textContent = g.label;
      dropdown.appendChild(lbl);
      g.items.forEach(([id, name, banner]) => {
        const opt = document.createElement('div');
        opt.className = 'saq-store-option';
        opt.dataset.id = id;
        opt.innerHTML = `${name} <span class="store-banner">(${banner})</span>`;
        opt.addEventListener('click', () => selectStore(id, name, banner));
        dropdown.appendChild(opt);
      });
    });
    if (!hasResults) {
      const empty = document.createElement('div');
      empty.className = 'saq-store-option';
      empty.textContent = 'Aucune succursale trouvee';
  ...
  function selectStore(id, name, banner) {
    input.value = `${name} (${banner})`;
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('#saqStoreAutocomplete')) dropdown.classList.remove('open');
  });

  // Expose a way to set value programmatically
  input._setStore = (id) => {
    const found = SAQ_STORES.find(s => s[0] === id);
    if (found) input.value = `${found[1]} (${found[2]})`;
  };
}
...
async function loadSAQStore() {
  try {
    ...
    const input = document.getElementById('saqStoreInput');
    if (input && input._setStore) input._setStore(saqStoreId);
  } catch {}
}
